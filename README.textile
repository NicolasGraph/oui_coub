...
"oui_coub":https://github.com/NicolasGraph/oui_coub is a "oui_player":https://github.com/NicolasGraph/oui_player v2+ extension to easily embed "Coub":https://coub.com/ provided videos in "Textpattern":https://textpattern.com/ powered websites.
...
oui_coub's minimum requirements:
...
# "Download":https://github.com/NicolasGraph/oui_coub/releases the compiled plugin file or the source to compile a customized file.
# Paste the content of the compiled plugin file under the _Admin > Plugins_ tab and click the _Upload_ button.
...
# Install oui_coub:
@$ composer require nicolasgraph/oui_coub@
...
bc. $ composer update nicolasgraph/oui_coub

h3. Uninstall

h4. From the admin interface

# Check the box on the left of the plugin row under the _Admin > Plugins_ tab.
# open the select list at the bottom of the plugins table and choose _Delete_.
# confirm the plugin deletion.

h4. Via Composer (not available yet)

bc. $ composer remove nicolasgraph/oui_coub

h2. Tags and parameters

h3. oui_coub vs oui_player

While the @<txp:oui_player />@ tag is able to embed any video from any extension-related provider, the @<txp:oui_coub />@ tag is a shortcut to embed Coub videos only. It is strictly equivalent to @<txp:oui_player provider="coub" />@.
...
h3. oui_if_coub vs oui_if_player

p. While the @<txp:oui_if_player>…</txp:oui_if_player>@ tag is able to check an URL against any URL from any extension-related provider, the @<txp:oui_if_coub>…</txp:oui_if_coub>@ tag is a shortcut to check an URL against Coub related URL schemes only. It is strictly equivalent to @<txp:oui_if_player provider="coub">…</txp:oui_if_player>@.
...
"oui_player":https://github.com/NicolasGraph/oui_player preference related field value: @https://coub.com/view/1asvzq@.

h3. Embed a player by defining its provider and a media ID through attributes

bc(language-markup). <txp:oui_player provider="coub" play="1asvzq" />

…or:

bc(language-markup). <txp:oui_coub play="1asvzq" />

h3. Conditionally embed a player using attributes

bc(language-markup).. <txp:oui_if_player provider="coub" play="https://coub.com/view/1asvzq">
    <txp:oui_player />
</txp:oui_if_player>

p. …or:

bc(language-markup).. <txp:oui_if_coub play="https://coub.com/view/1asvzq">
    <txp:oui_coub />
</txp:oui_if_coub>
...
oui_coub version 1.0.0-BETA1, Copyright (C) 2018 Nicolas Morand
...
* *oui_coub v1.0.0-BETA1* (2018-08-21)
